...
- `localnet start` waits until localnet is actually ready (`pid alive` + `127.0.0.1:3040` reachable), otherwise fails with diagnostics. The sequencer is daemonized via `setsid` so it survives shell/tmux session closure — closing the terminal or detaching a tmux session does not kill the localnet. The sequencer is daemonized via `setsid` so it survives shell/tmux session closure — closing the terminal or detaching a tmux session does not kill the localnet.
