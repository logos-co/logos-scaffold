...
- `basecamp modules` is the sole writer of the captured module + dependency set (`basecamp.state`). Zero-arg runs full auto-discovery: walks project flakes (root `.#lgx` first, else immediate sub-flakes), reads each source's `metadata.json` for declared `dependencies`, and resolves each dep name through a **fallback chain**:
  1. `[basecamp.dependencies]` in `scaffold.toml` (explicit project override — wins).
  2. A matching input in the source's own `flake.lock` (authoritative pin the project is already building against — rewritten to `github:<owner>/<repo>/<rev>#lgx`).
  3. Scaffold-level `BASECAMP_DEPENDENCIES` pin table (best-guess default; may be stale for projects that don't carry the dep as a flake input).
  4. Basecamp preinstalls (`capability_module` / `package_manager` / etc.) — silently skipped.
  5. Unknown — warn and skip with a hint to supply the pin.
  
  `--flake <ref>` / `--path <file>` captures the given set explicitly as project sources (manifest deps still resolved via the same fallback chain). `--show` prints the current capture without mutating. Always prints the previous set before overwriting so reverting is copy-paste. Captured entries are annotated with their provenance (e.g. `[dep `delivery_module` via path:/abs/tictactoe#lgx, pinned by /abs/tictactoe/flake.lock]`) so you can always tell where a pin came from. Project contract: see [docs/basecamp-module-requirements.md](./docs/basecamp-module-requirements.md).
