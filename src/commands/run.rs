...
fn run_pipeline_once(project: &Project, params: &PipelineParams) -> DynResult<()> {
    ...
    let skipped = if deploy_can_be_skipped(&current_hashes, &prior.program_hashes) {
        println!(
            "[5/{total_steps}] Deploy skipped (guest binaries + IDL unchanged; pass `--reset` or delete `.scaffold/state/run_deploy.json` to force a re-deploy)"
        );
        true
    } else {
...
fn reset_for_run(project: &crate::model::Project) -> DynResult<()> {
        ...
        LocalnetOwnership::Foreign => {
            ...
            bail!(
                "localnet port is in use by another process (pid={pid_display}).\n\
                 ...
                 or use `logos-scaffold run --reset` for a fresh-project reset."
            );
        }
