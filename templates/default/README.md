...
- `git`, `rustc`, `cargo`
- `wallet` binary (built locally by `logos-scaffold setup`)
- Docker or Podman (required by guest method builds)
- `logos-blockchain-circuits` for setup/localnet. Set
  `LOGOS_BLOCKCHAIN_CIRCUITS=/path/to/logos-blockchain-circuits` when it is not
  installed at `~/.logos-blockchain-circuits`.
...
- Circuits/prover failure: `logos-scaffold doctor`
- Guest build failure: verify Docker is running and reachable
